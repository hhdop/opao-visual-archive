...
function FuzzyText({
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      const resolvedFontSizeStr = `${numericFontSize}px`;
      const fontString = `${fontWeight} ${resolvedFontSizeStr} ${computedFontFamily}`;
      ...
      offCtx.font = `${fontWeight} ${resolvedFontSizeStr} ${computedFontFamily}`;
      ...
      if (gradient && Array.isArray(gradient) && gradient.length >= 2) {
        ...
        gradient.forEach((gradientColor, index) => grad.addColorStop(index / (gradient.length - 1), gradientColor));
      ...
      const run = (timestamp: number) => {
        ...
        if (direction === 'horizontal') {
          ...
          for (let x = 0; x < offscreenWidth + fuzzRange; x += 1) {
            ...
            const colData = ctx.getImageData(x, 0, 1, tightHeight + fuzzRange);
